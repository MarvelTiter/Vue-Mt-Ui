<template>
	<thead v-if="showHeader">
		<tr v-for="(tr,index) in tableRow" :key="index">
			<th v-for="(th,thIndex) in _getThead(index)" :key="thIndex" :rowspan="th.rowspan" :colspan="th.colspan" :class="[th.fixed,th.className]" :style="{textAlign:th.align}" :title="th.title?th.label:null" @mousedown="$parent._headMouseDown($event,thIndex)" @mousemove="$parent._headMouseMove($event,thIndex)">
				<label @click="$parent._handleSelectAll" :class="[selectChecked,prefixCls+'-checkbox']" v-if="th.type==='selection'">
					<span :class="`${prefixCls}-checkbox-inner`">{{th.label}}</span>
				</label>
